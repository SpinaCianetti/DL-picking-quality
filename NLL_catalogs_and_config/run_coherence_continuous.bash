...
# IMPORTANT: see also USER VARIABLES TO EDIT below in # COHERENCE CALCULATION BLOCK (cross-correlation coherence analysis)
#    and in # COHERENCE LOCATION BLOCK (")cross-correlation coherence analysis)

# identifier for a particular run
RUN_NAME=20250224C_cont_coherence
...
LOC_PATH=/Users/anthony/work_temp/nlloc_tmp/Norcia_2016/20250224C_cont_SSST/Norcia_2016_Tan2021_smooth_SSST/loc_ssst_corr0/PN-IN_005_EVENTS_2016_304
...
cp -p run_coherence_continuous.bash ${OUTPATH_ROOT}
...
# =========================================================================================================================
# WAVEFORM RETRIEVAL BLOCK
# =========================================================================================================================
# get waveforms (usually does not need to be repeated unless model, LOC_PATH events or TARGET_NSLCS changed)
if [ NO = YES ]; then
#if [ YES = YES ]; then
...
COHERENCE_MAX_DIST=3.5  # 20241026
...
#rm -r ${OUTPATH}
mkdir -p ${OUTPATH}
cp -p run_coherence_continuous.bash  ${OUTPATH}
...
echo "${PLOT_APP} ${OUTPATH}/*.png"
${PLOT_APP} ${OUTPATH}/*.png
...
parallel_task() {
    # run NLLoc
    NLLoc $1
    # remove last* output, not needed, doubles output size
    rm $2/last*
}
(
...
LOCHYPOUT SAVE_NLLOC_ALL  NLL_FORMAT_VER_2  SAVE_FMAMP
${LOC_GRID}
${LOC_SEARCH}
...
END
    ...
    # wait before running new instance of NLLoc until number running < N_PROCESS
    declare -i NUM_NLLOC=$(ps | grep "NLLoc" | wc -l)
    while [ ${NUM_NLLOC} -gt ${N_PROCESS} ] ; do
        sleep 0.1
    	NUM_NLLOC=$(ps | grep "NLLoc" | wc -l)
    done
    parallel_task ${TEMP_DIR}/NLL.${LOC_ROOT}_coherence_${CRUN_COUNT}.conf ${OUTPATH_COH}/${CRUN_COUNT} &
...
cd ${OUTPATH_COH}
find . -name "${LOC_ROOT}.sum.grid0.loc.hyp" -print0 | sort -z | xargs -0 cat > ${LOC_ROOT}.sum_ALL.grid0.loc.hyp
find . -name "${LOC_ROOT}.sum.grid0.loc.fmamp" -print0 | sort -z | xargs -0 cat > ${LOC_ROOT}.sum_ALL.grid0.loc.fmamp
cd ${WORK_DIR}
